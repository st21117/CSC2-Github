...
def print_julies_tracker():
    ...
    while name_count < total_entries:
        ...
        Label(main_window, text=(julies_tracker[name_count][1])).grid(column=2, row=name_count+8)
...
def check_inputs():
    ...
    global julies_tracker, entry_name, entry_item_hired, entry_no_item_hired, entry_receipt_number
    input_check = 0
    ...
    if (entry_no_item_hired.get().isdigit()):
        if int(entry_no_item_hired.get()) < 1 or int(entry_no_item_hired.get()) > 500:
            Label(main_window, fg="red", text="1-500 only") .grid(column=2, row=2)
            input_check = 1

    # Check that receipt number is not blank, set error if blank
...
def append_name():
    ...
    julies_tracker.append([entry_name.get(), entry_item_hired.get(), entry_no_item_hired.get(), entry_receipt_number.get()])
